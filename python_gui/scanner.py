...
class scanner:
    ...
    def __init__(self,path,k=1):
        ...
        if self.img.shape[0] > 2500 or self.img.shape[1] > 2500 :
            self.img = cv2.resize(self.img,(self.img.shape[0],self.img.shape[1]))
    ...
    def preprocessing(self):
        ...
        otsu_thresh_val,_ = cv2.threshold(img_blur, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        ...
        outputRect = np.array([[0, 0],[self.img.shape[0] - 1, 0],[self.img.shape[0] - 1, self.img.shape[1] - 1],[0, self.img.shape[1] - 1]], dtype = "float32")

        perspectiveMatrix = cv2.getPerspectiveTransform( inputRect, outputRect )
        warp_output = cv2.warpPerspective(self.img,perspectiveMatrix,(self.img.shape[0],self.img.shape[1]))

        size = int(warp_output.shape[0]*warp_output.shape[1]/2188)
        if size%2 == 0:
            size += 1

        binary_output = cv2.adaptiveThreshold( warp_output , 255 , cv2.ADAPTIVE_THRESH_MEAN_C , cv2.THRESH_BINARY , size , 0 )
    ...
    def getDigits(self):
        ...
        for x in range(9):
            for y in range(9):
                ...
                for i in range(s):
                    for j in range(s):
                        if i + int(900*x/9) < 900 and j + int(900*y/9) < 900 :
                            elm[i][j] = square[i+int(900*x/9)][j + int(900*y/9)]
                        else:
                            elm[i][j] = 0

                elm = cv2.convertScaleAbs(elm)
                
                contours, _ = cv2.findContours(elm, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                ...
                elm = elm[bounding_rect[1]+5:bounding_rect[1]+bounding_rect[3]-5,bounding_rect[0]+5:bounding_rect[0]+bounding_rect[2]-5]

                fin = cv2.resize(elm,(20,20))
                self.grid[x][y] = int(self.getNum(fin))

"""
s = scanner("/home/zoh/Programming_Data/sudoku_project_attempts/python/Pics/15.jpg")
