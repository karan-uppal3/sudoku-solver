...
class gui:
    ...
    def solution(self):
        ...
        if ifSolved == True:
            ...
            Label(root,text="Unable to solve").grid(row=8,column=10)
...
for i in range(3):
    for j in range(3):
        ...
        for k in range(3):
            for l in range(3):
                g.grid[i*3+k][j*3+l] = Entry(frame_temp,width=1,borderwidth=5)
                g.grid[i*3+k][j*3+l].grid(row=i*3+k,column=j*3+l,padx=5,pady=5)
        frame_temp.grid(row=i,column=j)

frame.grid(row=0,column=0,rowspan=10,columnspan=10)

Label(root,text="This is the Sudoku Solver. Using the \n'Select Image' button, select a picture \nof a sudoku. Then click the 'Solve'\n button to solve the puzzle.",anchor=W).grid(row=0,column=10,rowspan=4,columnspan=2,pady=15)

select_img_button = Button(root,text="Select Image",padx=120,pady=5,anchor=E,command=g.openfile)
select_img_button.grid(row=5,column=10,padx=15,columnspan=2)

solve_button = Button(root,text="Solve",padx=170,pady=5,command=g.solution)
solve_button.grid(row=6,column=10,padx=30,columnspan=2)

clear_button = Button(root,text="Clear",padx=170,pady=5,command=g.clear)
clear_button.grid(row=7,column=10,padx=30,columnspan=2)

quit_button = Button(root,text="Quit",padx=180,pady=5,command=root.destroy)
quit_button.grid(row=8,column=10,padx=30,columnspan=2)
